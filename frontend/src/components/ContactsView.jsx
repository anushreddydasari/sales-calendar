...
export default function ContactsView() {
  ...
  /* Filter contacts by team + source + lifecycle + status */
  const filteredContacts = useMemo(() =>
    contacts.filter((c) => {
      ...
      if (selectedSource !== 'all') {
        if ((p.lead_source || '') !== selectedSource) return false;
      }
    ...
    [contacts, ownerMap, selectedTeam, selectedSource, selectedLifecycle, selectedStatus]
  );
      ...
      <div className="cv-header">
        <div>
          ...
          <div className="cv-subtitle">
            {period.label} &middot; {stats.total} contacts created
          </div>
        </div>
        <div className="cv-filters">
          ...
          <div className="cv-fg">
            <label className="cv-fl">SOURCE</label>
            <select className="cv-sel" value={selectedSource} onChange={(e) => setSelectedSource(e.target.value)}>
              <option value="all">All Sources</option>
              {leadSources.map((s) => (
                <option key={s.id} value={s.id}>
                  {s.id === '__none' ? 'No Source' : (LEAD_SOURCE_LABELS[s.id] || s.id)} ({s.count})
                </option>
              ))}
            </select>
          </div>
