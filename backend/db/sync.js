...
async function syncContacts(db, token) {
  ...
  const properties = [
    ...
    'lifecyclestage', 'hs_lead_status', 'lead_source', 'lead_category',
  ...
  const upsert = db.prepare(`
    INSERT OR REPLACE INTO contacts (id, firstname, lastname, email, company, createdate, lifecyclestage,
      hs_lead_status, lead_source, lead_category, hubspot_owner_id, num_associated_deals, num_contacted_notes, num_notes, createdAt, updatedAt, archived)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  const tx = db.transaction((rows) => {
    ...
    for (const c of rows) {
      ...
      upsert.run(c.id, p.firstname, p.lastname, p.email, p.company, p.createdate,
        p.lifecyclestage, p.hs_lead_status, p.lead_source, p.lead_category,
